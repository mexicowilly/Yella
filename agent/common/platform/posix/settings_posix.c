...
void yella_initialize_platform_settings(void)
{
    ...
    yella_settings_set_dir(u"agent", u"data-dir", u"/var/lib/yella");
    yella_settings_set_dir(u"agent", u"spool-dir", u"/var/spool/yella");
    inst = yella_from_utf8(YELLA_VALUE_STR(YELLA_INSTALL_PREFIX));
    dir = udsnew(inst);
    dir = udscat(dir, u"/plugin");
    yella_settings_set_dir(u"agent", u"plugin-dir", dir);
    udsfree(dir);
    dir = udsnew(inst);
    dir = udscat(dir, u"/bin");
    yella_settings_set_dir(u"agent", u"bin-dir", dir);
    udsfree(dir);
    free(inst);
}
